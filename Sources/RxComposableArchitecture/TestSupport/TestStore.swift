...
public final class TestStore<State, Action, ScopedState, ScopedAction, Environment> {
    ...
    /// ``receive(_:_:file:line:)``, it will equal the `inout` state passed to the
    /// closure.
    ...
    private var _environment: TaskBox<Environment>
    ...
    internal init(
        _environment: TaskBox<Environment>,
        file: StaticString,
...
extension TestStore {
    ...
    public func skipInFlightEffects(
        ...
        for effect in self.reducer.inFlightEffects {
            _ = Effect<Never>.cancel(id: effect.id).subscribe(onNext: { _ in })
        }
