...
extension Effect {
    ...
    public func concatenate(with other: Self) -> Self {
        ...
        case (.observable, .observable), (.run, .observable), (.observable, .run):
            return Self(
                operation: .observable(
                    Observable.concat([self.asObservable(), other.asObservable()])
                )
            )
        case let (.run(lhsPriority, lhsOperation), .run(rhsPriority, rhsOperation)):
            return Self(
                operation: .run { send in
                    if let lhsPriority = lhsPriority {
                        await Task(priority: lhsPriority) { await lhsOperation(send) }
                            .cancellableValue
                    } else {
                        await lhsOperation(send)
                    }
                    if let rhsPriority = rhsPriority {
                        await Task(priority: rhsPriority) { await rhsOperation(send) }
                            .cancellableValue
                    } else {
                        await rhsOperation(send)
                    }
    ...
    public func map<T>(_ transform: @escaping (Action) -> T) -> Effect<T> {
        ...
        case let .observable(observable):
            ...
            return .init(operation: .observable(observable.map(transform)))
